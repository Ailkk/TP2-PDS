...
public class Llvm {
	...
	static public class DefineFunc extends Instruction{
		...
		HashMap<String,Type> paramType;

		public DefineFunc(String n, Type tN, List<String> p, HashMap<String,Type> tP) {
			this.name=n;
			this.nameType=tN;
			this.param=p;
			this.paramType=tP;
		...
		public String toString() {
			...
			String res = "define " + this.nameType + " " + this.name + "(";
			if(!param.isEmpty()) {

				Iterator<String> it = param.iterator();
				String parametre = it.next();

				res += this.paramType.get(parametre) + " %" + parametre;
	...
	static public class AppelFunc extends Instruction{
		...
		public AppelFunc(String n, Type tN, List<String> p, HashMap<String,Type> tP, String v) {
			this.name=n;
			this.nameType=tN;
			this.param=p;
			this.paramType=tP;
		...
		public String toString() {
			...
			String param = null;
			String res = (this.ValRes.length() > 0)? this.ValRes+" = " : "";

			for(String attribut : this.param) {
				if(param == null) param = this.ValRes/*.get(attribut)*/ + " " + attribut;
				else param += ", " + this.ValRes/*.get(attribut)*/ + " " + attribut;
			}
			return res + "call " + this.nameType + " " + this.name + "(" + param + ")\n";
		}
	...
	static public class AllocationFunc extends Instruction {
		//Nom Variable
		String ident;
		...
		public AllocationFunc(String ident,Type type) {
			this.type = type;
			this.ident = ident;
		}
